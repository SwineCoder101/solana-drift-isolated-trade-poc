...
exports.CrossbarClient = exports.CrossbarNetwork = void 0;
 ...
 */
var CrossbarNetwork;
(function (CrossbarNetwork) {
    CrossbarNetwork["SolanaMainnet"] = "mainnet";
    CrossbarNetwork["SolanaDevnet"] = "devnet";
})(CrossbarNetwork || (exports.CrossbarNetwork = CrossbarNetwork = {}));
class CrossbarClient {
    ...
    async fetchSolanaUpdates(network, feedpubkeys, payer, numSignatures) {
        try {
            ...
            const updates = response.map((update) => ({
                ...update,
                pullIxns: update.pullIxns.map(instructions_js_1.IxFromHex),
            }));
            return updates;
        }
        catch (err) {
     ...
     */
    async fetchOracleQuote(aggregatorIds, network = 'mainnet') {
    ...
    async fetchGateways(network = 'mainnet') {
        try {
            const gateways = await axios_1.default
     ...
     * ```
     */
    async fetchGateway(network) {
        const targetNetwork = network !== null && network !== void 0 ? network : this.network;
        ...
        const gateway = new gateway_js_1.Gateway(gatewayUrls[0]);
     ...
     * );
     */
    async simulateFeed(feedOrHash, includeReceipts, variableOverrides, network) {
        try {
            ...
            if (typeof feedOrHash === 'string') {
                ...
                feed = protos_js_1.OracleFeed.decodeDelimited(protoBytes);
            }
            else {
                feed = feedOrHash;
            }
            // Encode the OracleFeed proto to bytes
            const protoBytes = protos_js_1.OracleFeed.encode(feed).finish();
            // Convert to base64
    ...
    async simulateFeeds(feedHashes, includeReceipts) {
        if (!feedHashes || feedHashes.length === 0)
            throw new Error('At least one feed is required');
        return await Promise.all(feedHashes.map(feedHash => this.simulateFeed(feedHash, includeReceipts)));
    }
    /**
    ...
    async simulateOracleFeeds(feedHashes, includeReceipts, variableOverrides, network) {
        if (!feedHashes || feedHashes.length === 0)
            throw new Error('At least one feed is required');
        return await Promise.all(feedHashes.map(feedHash => this.simulateFeed(feedHash, includeReceipts, variableOverrides, network)));
    }
}
