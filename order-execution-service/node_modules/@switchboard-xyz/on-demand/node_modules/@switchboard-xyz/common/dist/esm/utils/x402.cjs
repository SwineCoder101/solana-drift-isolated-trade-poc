...
exports.X402Utils = exports.X402Error = void 0;
exports.handleX402Request = handleX402Request;
exports.createX402PaymentHeader = createX402PaymentHeader;
const axios_js_1 = require("./axios.cjs");
 ...
 * ```
 */
async function handleX402Request(httpTask, keypair) {
    var _a;
    // Validate httpTask
    if (!httpTask.url) {
    ...
    const response = await (0, axios_js_1.fetch)(httpTask.url, {
        method,
        headers,
        body: httpTask.body,
    });
    ...
    if (response.status === 402) {
        ...
        const messageBytes = buffer_1.Buffer.from(messageToSign, 'utf-8');
        // Sign with ed25519 using tweetnacl
        let signature;
        if (keypair instanceof web3_js_1.Keypair) {
            signature = tweetnacl_1.sign.detached(messageBytes, keypair.secretKey);
        }
        else if (keypair instanceof Uint8Array) {
            // Assume it's a secret key
            signature = tweetnacl_1.sign.detached(messageBytes, keypair);
        }
        else {
            throw new X402Error('Invalid keypair: must be Solana Keypair or Uint8Array secret key');
        }
        // Encode signature as base64
        const signatureBase64 = buffer_1.Buffer.from(signature).toString('base64');
        return {
            is402: true,
 ...
 * ```
 */
function createX402PaymentHeader(paymentDetails, signature, publicKey) {
    const payload = {
        payment_details: paymentDetails,
        signature,
        public_key: typeof publicKey === 'string'
            ? publicKey
            : buffer_1.Buffer.from(publicKey).toString('base64'),
    };
    return buffer_1.Buffer.from(JSON.stringify(payload)).toString('base64');
}
/**
