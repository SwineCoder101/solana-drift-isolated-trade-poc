...
__exportStar(require("./types/gateway.cjs"), exports);
const TIMEOUT = 10000;
const axiosClient = (() => {
    let instance;
...
function encodeJobs(jobs) {
    return jobs.map(job => index_js_1.OracleJobUtils.serializeOracleJob(job).toString('base64'));
}
/**
...
class Gateway {
     ...
     * ```
     */
    constructor(gatewayUrlOrInstance) {
        this.gatewayUrl =
    ...
    async fetchSignaturesConsensus(params) {
        ...
        const feedRequests = isV1
            ...
            : feedConfigs.map(config => ({
                feed_proto_b64: index_js_1.OracleFeedUtils.serializeOracleFeed(config.feed).toString('base64'),
            }));
     ...
     * ```
     */
    async fetchQuote(crossbar, feedHashes, numSignatures = 1, variableOverrides) {
        const feedRequests = await Promise.all(feedHashes.map(async (hash) => {
            try {
