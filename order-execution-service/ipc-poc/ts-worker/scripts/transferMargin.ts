...
import { initDrift, buildTransferIsolatedMarginTx } from '../src/drift.js';
...
function parseArgs(): CliArgs {
	...
	if (isNaN(delta)) throw new Error('Invalid --delta (must be a number)');
...
async function main() {
	...
	await initDrift();
	const { txBase64 } = await buildTransferIsolatedMarginTx({
		wallet: args.wallet,
		market: args.market,
		delta: args.delta,
	});
	...
	console.log('Transfer margin:', args.delta > 0 ? 'Deposit' : 'Withdraw', Math.abs(args.delta));
