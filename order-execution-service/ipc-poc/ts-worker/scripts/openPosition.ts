...
import { initDrift, buildOpenIsolatedTx } from '../src/drift.js';
...
function parseArgs(): CliArgs {
	...
	return {
		...
		size: Number(args.size ?? '0.1'),
		leverage: Number(args.leverage ?? '5'),
		margin: Number(args.margin ?? '1'),
	};
}
...
async function main() {
	...
	await initDrift();
	const { txBase64, meta } = await buildOpenIsolatedTx({
		wallet: args.wallet,
		market: args.market,
		size: args.size,
